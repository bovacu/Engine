...
namespace engine {
    ...
    Component* GameObject::removeComponent(Component* _component) {
        LOG_CRITICAL_CORE("REMOVE FROM GAME_OBJECT IS NOT FUNCTIONAL YET");
        return nullptr;
    }

    BoxCollider* GameObject::addBoxCollider(const Size& _size, bool _isGhost) {
    ...
    CircleCollider* GameObject::addCircleCollider(float _radius, bool _isGhost) {
    ...
    PolygonCollider* GameObject::addPolygonCollider(const std::vector<Vec2f>& _vertices, bool _isGhost) {
    ...
    PhysicsBody* GameObject::addPhysicsBody() {
        auto _physicsBody = PhysicsBody::create(this, this->transform.position);
        this->components.emplace_back(_physicsBody);
        return _physicsBody;
    }

    Sprite* GameObject::addSprite(const std::shared_ptr<TextureRegion>& _texture) {
